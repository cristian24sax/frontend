...
import { fetchCourseList } from "@/modules/dashboard/service/dashboard.service";
...
const AsideBar = () => {
  ...
  useEffect(() => {
    const dataUser = localStorage.getItem("admin");
    if (dataUser === "true") {
      setUserRole("admin");
      return;
    }
              ...
              <div key={i} className={`group flex flex-col gap-1 ${menu.margin && "mt-5"}`}>
                <div className="flex items-center text-sm gap-3.5 font-medium p-2 hover:bg-gray-800 rounded-md">
                  <Link href={menu.link as any} className="flex items-center">
                    ...
                    <span style={{ transitionDelay: `${i + 3}00ms` }} className={`whitespace-pre duration-500 ${!open ? "opacity-0 translate-x-28 overflow-hidden" : "opacity-100 translate-x-0"}`}>
                      {menu.name}
                    </span>
                  </Link>
