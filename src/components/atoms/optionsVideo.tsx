...
export const OptionsComponent = ({ item, index }: Props) => {
  ...
  return (
    <div onClick={() => handleOptions(item.id)} className={`flex items-center gap-3 p-2  pr-1 rounded-md hover:bg-gray-200  text-black transition-colors cursor-pointer ${value === item.id || valueNew === item.id ? "bg-neutral-900 " : ""}`}>
      <div className="grow">
